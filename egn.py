...
class Projector(nn.Module):
    def __init__(
        ...
        player,
        linear_projection,
        num_genes, # number of genes to be predicted (not included in original EGN code)
        dropout = 0.,
    ):
        super().__init__()
...
class EGN(nn.Module):
    def __init__(self, bhead, bdim, bfre, mdim, player, linear_projection,
                 image_size = 256, patch_size = 32, num_classes = 256, dim = 1024, 
        ...
        self.transformer = Transformer(mdim, dim, player, depth, linear_projection, heads, dim_head, mlp_dim, dropout, bhead, bdim, bfre, num_genes = num_classes)
