import torch
import torch.nn as nn
import time
import datetime
import subprocess 
import pytorch_lightning as pl
from cosine_annealing_warmup import CosineAnnealingWarmupRestarts
import os
import numpy as np
# Using spared library
from spared.metrics import get_metrics
import wandb

def calculate_metrics(y_true, y_pred, type, mask):

    metrics = get_metrics(y_true, y_pred, mask)
    log_dict = { f'{type}_{key}': val for key, val in metrics.items() }
    wandb.log(log_dict)
    
    return metrics

def compute_correlations(labels, preds, return_detail = False):
    device = labels.device
    
    labels = labels.detach().cpu().numpy()
    preds = preds.detach().cpu().numpy()
    corr = np.nan_to_num([np.corrcoef(labels[:,i], preds[:,i])[0,1] for i in range(labels.shape[1])], nan = -1).tolist()
    if return_detail:
        return corr
    corr = np.mean(corr)
    return torch.FloatTensor([corr]).to(device)

def to_gene(gene_corr):
    output = ""
    for i,j in gene_corr:
        output += "[%s : %.4f] " % (i,j)
    return output

def pearsonr(x, y):

    mean_x = torch.mean(x)
    mean_y = torch.mean(y)
    xm = x.sub(mean_x)
    ym = y.sub(mean_y)
    r_num = xm.dot(ym)
    r_den = torch.norm(xm, 2) * torch.norm(ym, 2)
    r_val = r_num / (r_den + 1e-8)
    r_val = torch.nan_to_num(r_val,nan=-1)
    return r_val


class TrainerModel(pl.LightningModule):
    
    def __init__(self, config, model):
        super().__init__()
        self.model = model
        self.config = config
        self.criterion = nn.MSELoss()
        self.automatic_optimization = False
        self.min_loss  = float("inf")
        self.max_corr  = float("-inf")
        self.max_eval_corr = float("-inf")
        self.min_eval_loss = float("inf")
        self.start_time  = None
        self.last_saved = None

        self.best_MSE = float("inf")

        self.validation_step_outputs = []
        self.test_step_outputs = []

        self.best_ckpts_path = ''
    
    def correlationMetric(self, x, y):
      corr = 0
      for idx in range(x.size(1)):
          corr += pearsonr(x[:,idx], y[:,idx])
      corr /= (idx + 1)
      return (1 - corr).mean()
    
    def training_step(self, data, idx):
        
        if self.current_epoch == 0 and idx == 0:
            self.start_time  = time.time()
        
        optimizer = self.optimizers()
        
        pred_count = self.model(data)
        loss = self.criterion(pred_count, data["count"])
        corrloss = self.correlationMetric(pred_count, data["count"])

        full_metrics = calculate_metrics(data["count"], pred_count, type='train', mask=data["mask"])
        
        optimizer.zero_grad()
        self.manual_backward(loss + corrloss * 0.5)
        optimizer.step()
        
        lr_scheduler = self.lr_schedulers()
        lr_scheduler.step()
        
        self.produce_log(loss.detach(),corrloss.detach(),data,idx)
               
    def produce_log(self, loss, corr, data, idx):
        
        train_loss = self.all_gather(loss).mean().item()
        train_corr = self.all_gather(corr).mean().item()
        
        self.min_loss   = min(self.min_loss, train_loss)
        
        if self.trainer.is_global_zero and loss.device.index == 0 and idx % self.config.verbose_step == 0:
            
            current_lr = self.optimizers().param_groups[0]['lr']
            
            len_loader = self.config.max_steps
            
            batches_done = self.current_epoch  * len_loader + idx + 1
            batches_left = self.trainer.max_epochs * len_loader - batches_done
            time_left    = datetime.timedelta(seconds = batches_left * (time.time() - self.start_time) / batches_done)
                    
            self.config.logfun(
                        "[Epoch %d/%d] [Batch %d/%d] [Loss: %f, Corr: %f, lr: %f] [Min Loss: %f] ETA: %s" % 
                        (self.current_epoch,
                         self.trainer.max_epochs,
                         idx,
                         len_loader,
                         train_loss,
                         train_corr,
                         current_lr,
                         self.min_loss,
                         time_left
                            )
                        
                        )
            
        
    def validation_step(self, data, idx):
        pred_count = self.model(data)
        self.validation_step_outputs.append((pred_count, data["count"], data["mask"]))
        return pred_count, data["count"], data["mask"]
        
    def on_validation_epoch_end(self):
        
        logfun = self.config.logfun
        all_preds = self.validation_step_outputs
        
        pred_count = torch.stack([i[0] for i in all_preds])
        count = torch.stack([i[1] for i in all_preds])
        mask =  torch.stack([i[2] for i in all_preds]) #for mask consideration

        pred_count = self.all_gather(pred_count).view(-1,256)
        count = self.all_gather(count).view(-1,256)
        mask = self.all_gather(mask).view(-1,256) #for mask consideration
        
        total_loss = self.criterion(pred_count,count).item()
        gene_corr = compute_correlations(count, pred_count, True)
        corr = np.mean(gene_corr)
        gene_corr = sorted(list(zip(self.config.filter_name, gene_corr)), key = lambda x : x[1])
        
        full_metrics = calculate_metrics(count, pred_count, type='val', mask=mask)
        PCC_Gene = full_metrics['PCC-Gene']
        PCC_Patch = full_metrics['PCC-Patch']
        R2_Gene = full_metrics['R2-Gene']
        R2_Patch = full_metrics['R2-Patch']
        MSE = full_metrics['MSE']
        MAE = full_metrics['MAE']
        
        if self.trainer.is_global_zero and self.trainer.num_devices != 0:
            for line in subprocess.check_output(["nvidia-smi"]).decode("utf-8").split("\n"):
                self.config.logfun(line)
            
            # Save best model based on MSE
            if MSE < self.best_MSE:
                self.best_ckpts_path = self.save(self.current_epoch, total_loss,corr)
                best_metrics = get_metrics(count, pred_count, mask=mask)
                bestval_dict={f'best_val_{key}': val for key, val in best_metrics.items()}
                bestval_dict["Epoch"]=self.current_epoch
                wandb.log(bestval_dict)

                self.best_ckpts_path = self.save(self.current_epoch, total_loss,corr) 

            self.max_eval_corr = max(self.max_eval_corr,corr)
            self.min_eval_loss = min(self.min_eval_loss, total_loss)
            self.best_MSE = min(self.best_MSE, MSE)
                            
            logfun("==" * 25)
            logfun(
                "[Corr :%f, Loss: %f] [Min Loss :%f, Max Corr: %f] [Best MSE: %f]" %
                (corr,
                 total_loss,
                 self.min_eval_loss,
                 self.max_eval_corr,
                 self.best_MSE
                 )
                )  

            logfun("==" * 25)
            logfun(
                "[PCC-Gene :%f, PCC-Patch: %f, R2-Gene: %f, R2-Patch: %f, MSE: %f, MAE: %f]" %
                (PCC_Gene,
                 PCC_Patch,
                 R2_Gene,
                 R2_Patch,
                 MSE,
                 MAE
                 )
                )

            logfun("Top 10 gene corr")
            logfun(to_gene(gene_corr[-10:][::-1]))
            logfun("==" * 25)
            logfun("End Evaluation")

        self.validation_step_outputs.clear()
        
    def save(self, epoch, loss, acc):
        
        self.config.logfun(self.last_saved)
        if self.last_saved != None:
            os.remove(self.last_saved)
        output_path = os.path.join(self.config.store_dir, "%d_%f_%f.pt" % (epoch,loss,acc))
        self.last_saved = output_path

        state_dict = self.model.state_dict()
        state_dict = {f'model.{key}': val for key, val in state_dict.items()}
        ckpt_dict = {'pytorch-lightning_version':'2.1.3', 'state_dict':state_dict}
        torch.save(ckpt_dict, output_path)
        self.config.logfun("EP:%d Model Saved on:" % epoch, output_path)
        return output_path
                      
    def configure_optimizers(self):
        
        optimizer = torch.optim.AdamW(
                            self.parameters(),
                            lr = self.config.lr,
                            betas = (0.9, 0.999),
                            weight_decay = self.config.weight_decay,
            )
        lr_scheduler = CosineAnnealingWarmupRestarts(
            optimizer,
            first_cycle_steps=self.config.max_steps//2,
            cycle_mult=1.0,
            max_lr=self.config.lr,
            min_lr=self.config.lr/5,
            warmup_steps= 0
        )
        
        return {'optimizer': optimizer, 'lr_scheduler': lr_scheduler}     

    def test_step(self, data, idx):
        pred_count = self.model(data)
        self.test_step_outputs.append((pred_count, data["count"], data["mask"]))
        return pred_count, data["count"], data["mask"]
        
    def on_test_epoch_end(self):
        
        logfun = self.config.logfun
        all_preds = self.test_step_outputs
        
        pred_count = torch.stack([i[0] for i in all_preds])
        count = torch.stack([i[1] for i in all_preds])
        mask =  torch.stack([i[2] for i in all_preds]) #for mask consideration

        pred_count = self.all_gather(pred_count).view(-1,256)
        count = self.all_gather(count).view(-1,256)
        mask = self.all_gather(mask).view(-1,256) #for mask consideration
        
        total_loss = self.criterion(pred_count,count).item()
        gene_corr = compute_correlations(count, pred_count, True)
        corr = np.mean(gene_corr)
        gene_corr = sorted(list(zip(self.config.filter_name, gene_corr)), key = lambda x : x[1])

        full_metrics = calculate_metrics(count, pred_count, type='test', mask=mask)
        PCC_Gene = full_metrics['PCC-Gene']
        PCC_Patch = full_metrics['PCC-Patch']
        R2_Gene = full_metrics['R2-Gene']
        R2_Patch = full_metrics['R2-Patch']
        MSE = full_metrics['MSE']
        MAE = full_metrics['MAE']
                        
        logfun("==" * 25)
        logfun(
            "[Corr :%f, Loss: %f, MSE: %f]" %
            (corr,
            total_loss,
            self.best_MSE
            )
            )  

        logfun("==" * 25)
        logfun(
            "[PCC-Gene :%f, PCC-Patch: %f, R2-Gene: %f, R2-Patch: %f, MSE: %f, MAE: %f]" %
            (PCC_Gene,
            PCC_Patch,
            R2_Gene,
            R2_Patch,
            MSE,
            MAE
            )
            )

        logfun("Top 10 gene corr")
        logfun(to_gene(gene_corr[-10:][::-1]))
        logfun("==" * 25)
        logfun("End Evaluation")

        self.test_step_outputs.clear()                  